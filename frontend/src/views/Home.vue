...
<div class="container font-base mx-auto">
    ...
    <div v-if="bookmarks.length" class="mx-4 lg:mx-0 grid lg:grid-cols-4 mt-6 gap-5 gap-y-4">
        <div v-for="bookmark in bookmarks" class="bg-[#111111] flex flex-col gap-8 overflow-hidden text-stone-400 h-32 rounded-lg">
            <div class="flex justify-between items-center ml-5 mr-5 mt-2">
                <div class="flex gap-2 mt-2 items-center">
                    <img v-if="bookmark.thumbnail" class="w-12 h-12 object-cover rounded" :src="bookmark.thumbnail" alt="">
                    <svg v-else xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-12 h-12 text-stone-500">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-1.605.42-3.113 1.157-4.418" />
                    </svg>

                    <h3>{{ bookmark.site_name.charAt(0).toUpperCase() + bookmark.site_name.slice(1) }}</h3>
                </div>
                ...
                <div class="lg:fixed ml-12 mt-1 w-56 text-right">
                    <Menu as="div" class="relative inline-block text-left">
                        <div>
                            <MenuButton class="inline-flex w-full justify-center text-sm font-medium">
                                <span class="bg-[#191919] text-stone-500 px-1.5 py-1.5 rounded-lg">
                        ...
                        <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0">
                            <MenuItems class="absolute right-0 mt-2 w-40 origin-top-right rounded-lg bg-[#191919]  shadow-lg">
                                <div class="px-1 py-1">
                                    <MenuItem v-slot="{ active }">
                                    <button @click="editBookmark(bookmark)" :class="[
                  active ? 'bg-[#6c53cd]' : 'text-white',
                  'group flex gap-2 w-full items-center rounded-lg px-2 py-2 text-sm',
                ]">
                  ...
                  active ? 'bg-[#6c53cd]' : 'text-white',
                  'group flex w-full gap-2 items-center rounded-lg px-2 py-2 text-sm',
                ]">
                                        <svg :active="active" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
        ...
        <ul v-if="bookmarks.length" class="mt-2 text-sm font-base inline-flex -space-x-px items-center divide-x divide-stone-900">
